...
# epsilon = propotion of real images in mixed images
...
def gradient_penalty(gradient):
    ...
    distance = torch.ones_like(gradient_norm) - gradient_norm
    quadratic_distance = distance**2
    penalty = torch.mean(quadratic_distance)
    return penalty


# Loss(Generator) is negative of fake images' score
# negative: score should be higher, loss should be lower
...
# the loss is calculated by maximizing the distance between the C(real images) and C(fake images),
# while also adding a gradient penalty
def get_crit_loss(crit_fake_pred, crit_real_pred, gp, c_lambda):
    ...
    '''
    pred_F = torch.mean(crit_fake_pred)
    pred_R = torch.mean(crit_real_pred)
    GP = torch.mean(gp) * c_lambda
    crit_loss = pred_F - pred_R + GP

    return crit_loss


# the critic is updated multiple times every time updating the generator,
# to help prevent the generator from overpowering the critic.
cur_step = 0
generator_losses = []
critic_losses = []
