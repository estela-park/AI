...
from tensorflow.keras.layers import Dense, Conv2D, MaxPool2D, Input, Dropout, GlobalAveragePooling2D, Layer
...
hl = MaxPool2D()(hl)
hl = Conv2D(64, (2, 2), padding='valid', activation='relu')(hl)
hl = Dropout(0.2)(hl)
hl = MaxPool2D()(hl)                         
...
print(x_train_ma.shape)

es = EarlyStopping(monitor='acc', patience=10, mode='max', verbose=1)

# hist = model.fit(x_train, y_train, epochs=100, batch_size=16, verbose=2, validation_split=0.25, callbacks=[es])
hist_std = model_std.fit(x_train_std, y_train, epochs=100, batch_size=16, verbose=2, validation_split=0.25, callbacks=[es])
hist_mm = model_mm.fit(x_train_mm, y_train, epochs=100, batch_size=16, verbose=2, validation_split=0.25, callbacks=[es])
hist_ma = model_ma.fit(x_train_ma, y_train, epochs=100, batch_size=16, verbose=2, validation_split=0.25, callbacks=[es])
